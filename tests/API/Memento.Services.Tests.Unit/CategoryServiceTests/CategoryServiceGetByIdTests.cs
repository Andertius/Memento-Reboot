...
public sealed class CategoryServiceGetByIdTests
{
    [Fact]
    public async Task Should_return_category_when_found()
    {
        ...
        A.CallTo(() => repository.GetById(1)).Returns(categoryEntity);

        var sevice = new CategoryService(repository);

        // Act
        var category = await sevice.GetById(1, CancellationToken.None);
    ...
    public async Task Should_return_null_when_not_found()
    {
        // Arrange
        var repository = A.Fake<ICategoryRepository>();
        A.CallTo(() => repository.GetById(1)).Returns<CategoryEntity?>(null);
